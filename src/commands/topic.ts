...
export async function handleTopicList(): Promise<void> {
  ...
  if (topics.length === 0) {
    console.log(
      chalk.yellow(
        "No topics found. Create one with `memo topic create <name>`",
      ),
    );
    return;
  }
...
export async function handleTopicShow(
  name: string,
  ...
  if (summary) {
    ...
    if (options?.showLlm && summary.generatedBy) {
      console.log(
        chalk.gray(
          `\nGenerated by: ${summary.generatedBy.provider}/${summary.generatedBy.model}`,
        ),
      );
    }
  }
...
export async function handleTopicRename(
  oldName: string,
  newName: string,
): Promise<void> {
  ...
  // Update frontmatter in all note files that reference this topic
  for (const note of notes) {
    const noteTopics = getTopicsForNote(note.id);
    const updatedTopicNames = noteTopics.map((t) =>
      t.id === topic.id ? newName : t.name,
    );
    const filePath = path.join(config.storePath, note.path);
    try {
      updateNoteFileFrontmatter(filePath, updatedTopicNames);
    } catch {
...
export async function handleTopicDelete(name: string): Promise<void> {
  ...
  for (const note of notes) {
    const noteTopics = getTopicsForNote(note.id);
    const remainingTopics = noteTopics
...
export async function handleTopicSummarize(name: string): Promise<void> {
  ...
  if (notes.length === 0) {
    console.log(
      chalk.yellow("No notes in this topic. Cannot generate summary."),
    );
    return;
  }

  const { content, metadata } = await provider.summarizeTopic(topic, notes);

  addSummaryVersion(topic.id, content, metadata);
...
export async function handleTopicHistory(name: string): Promise<void> {
  ...
  history.forEach((s) => {
    console.log(
      chalk.bold(
        `Version ${s.version} - ${s.generatedAt.toLocaleDateString()}`,
      ),
    );
...
export async function handleTopicPropose(): Promise<void> {
  ...
  proposals.forEach((proposal, i) => {
    console.log(chalk.bold(`${i + 1}. ${proposal.name}`));
    console.log(chalk.gray(`   ${proposal.description}`));
    if (proposal.sampleNotes.length > 0) {
      console.log(
        chalk.cyan(
          `   Sample notes: ${proposal.sampleNotes.slice(0, 3).join(", ")}`,
        ),
      );
    }
    console.log();
  ...
  const createInput = await p.text({
    message:
      "Create topics? Enter comma-separated numbers (or 'all' or 'none'):",
    placeholder: "all",
  });

  if (p.isCancel(createInput) || createInput === "none") {
  ...
  let toCreate: string[];
  if (createInput === "all") {
    toCreate = proposals.map((proposal) => proposal.name);
  } else {
