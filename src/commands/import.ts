...
export async function handleImport(
  ...
  const provider: LLMProvider = createProvider(config.llm);
  const llmAvailable = await provider.isAvailable();

  if (!llmAvailable) {
    console.log(
      chalk.yellow(
        "LLM not available. Notes will be imported without topic suggestions.",
      ),
    );
  }
...
async function importNotes(
  ...
  const discoveredFiles = globSync(["**/*.md", "**/*.txt"], {
    cwd: importPath,
    absolute: true,
  });
  ...
  s.message(
    `Found ${discoveredFiles.length} file${discoveredFiles.length === 1 ? "" : "s"} to import`,
  );

  const mode: "auto" | "default" | "interactive" = options.auto
    ? "auto"
    : options.default
      ? "default"
      : "interactive";
    ...
    try {
      ...
      if (options.default) {
        ...
        if (isTxtFile && llmAvailable) {
          stepNum++;
          s.message(
            getStepMessage("summarizing", relativePath, stepNum, totalSteps),
          );
          try {
            ...
            console.error(
              chalk.yellow(
                `Warning: Could not generate summary for ${note.title}: ${error}`,
              ),
            );
          }
        }
        ...
        stepNum++;
        s.message(
          getStepMessage("finalizing", relativePath, stepNum, totalSteps),
        );
      ...
      if (isTxtFile) {
        ...
        try {
          ...
          console.error(
            chalk.yellow(`Warning: Could not generate summary: ${error}`),
          );
        }
      }
      ...
      stepNum++;
      s.message(
        getStepMessage("classifying", relativePath, stepNum, totalSteps),
      );

      try {
        const noteForClassification = isTxtFile
          ? { ...note, content: summaryContent }
          : note;
        const result = await provider.classifyNote(
  ...
  s.stop(
    `Processed ${discoveredFiles.length} file${discoveredFiles.length === 1 ? "" : "s"}`,
  );
